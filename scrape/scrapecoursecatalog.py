...
#list of dictionaries to hold all of our info
weightMatrix = []
        ...
        if "(" in p_tag.text:
            if "." in p_tag.text:
                ...
                if courseID[0].isdigit():
                    weightMatrix.append({'Course ID': currentsite + ' ' + courseID,'Title': name ,'Units': units,'Description':description,'Prerequisites':preqs})
                else:
                    weightMatrix.append({'Course ID': courseID,'Title': name ,'Units': units,'Description':description,'Prerequisites':preqs})
