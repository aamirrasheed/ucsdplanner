    ...
    discussionTable.read({
        success: function(discussions) {
                ...
                if (loc < 0) {
                    ...
                    if (secLoc < 0) {
                        if (d.disc_seats_total > 0) {
                            seatsInSec.push(d.disc_seats_total);
                            sectionId.push(d.section_id);
                        }
                    }
                    else {
                        if (d.disc_seats_total > 0)
                            seatsInSec[secLoc] += d.disc_seats_total;
                    }                   
                }        
            }
            ...
            for (var j = 0; j < sectionId.length; j++) {
                ...
                var sql = "update course_section set course_section.sec_seats_total = case " + insert + " end where course_section.id in ('" + sectionId.join("', '") + "');";
                console.log(sql);
                mssql.query(sql,  {
        ...
        success: function(sections) {
            ...
            for (var i = 0; i < sections.length; i++) {
                ...
                if (loc < 0) {
                    ...
                    // if not present, insert
                    if (courseLoc < 0) {
                        seatsInCourse.push(s.sec_seats_total);
                        courseId.push(s.course_id);
                    }
                    else {
                        if (s.sec_seats_total > 0)
                            seatsInCourse[courseLoc] += s.sec_seats_total;
                    }                   
                }        
            }
            ...
            for (var j = 0; j < courseId.length; j++) {
                ...
                var sql = "update course set course.seats_total = case " + insert + " end where course.id in ('" + courseId.join("', '") + "');";
                mssql.query(sql, {
                    success:function(results) {
                    },
                    error: function(results) {
